...
<?php
	...
	$sql1="SELECT * FROM ordertable WHERE seller_username = 'KamalPerera' AND status = 'Pending' OR status = 'Delivery Ready' ORDER BY ord_Date DESC";
	$res1=mysqli_query($con,$sql1) or die(mysqli_error($con));

	$sql2="SELECT * FROM ordertable WHERE seller_username = 'KamalPerera' AND status = 'Dispatched' OR status = 'Delivery Accepted' ORDER BY ord_Date DESC";
...
?>
	...
	<div class="tab-content" id="orderTabs">
	  ...
	  <div id="pending" class="tab-pane fade in active">
		...
		?>
				<td></td>
				<td><input type="button" name="completeview" value="Complete Order" id="<?php echo $row[0]; ?>" class="complete_order btn btn-info btn-xs" /></td>
		<?php
			}
		...
		?>
				...
				<td><input type="button" name="proceedview" value="Proceed Order" id="<?php echo $row[0]; ?>" class="proceed_order btn btn-info btn-xs" /></td>
		<?php
			}
			else
			{
